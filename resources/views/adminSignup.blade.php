...
<div class='flex space-y-10 bg-c2/30 shadow-2xl rounded-xl p-10 flex-col lg:w-1/3 w-full lg:h-fit h-full'>
      <h3 class='text-c1 font-bold lg:text-2xl text-xl text-center'>@lang('lang.createNewAccount')</h3>
        <h3 class="text-c1 font-semibold text-center text-md">@lang('lang.alreadyAccount')<a href="{{url('/login/admin')}}" ><span class="text-red-400 hover:underline"> @lang('lang.login')</span></a></h3>
      {!! Form::open(['url'=>url('/signup/admin'),'method'=>'post','class'=>'flex flex-col']) !!}
